...
public class Processor {
	...
	private static Class<? extends Emitter> emitterClazz = Emitter.class;
	/** The reader. */
	...
	public static void setEmitterClazz(Class<? extends Emitter> emitterClazz) {
		Processor.emitterClazz = emitterClazz;
	}
	...
	protected Processor(final Reader reader, final Configuration config) {
		...
		try {
			this.emitter = emitterClazz.getConstructor(Configuration.class).newInstance(this.config);
		}
		catch (Exception e) {
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final File file, final String encoding, final Decorator decorator, final boolean safeMode) throws IOException {
		return process(file, Configuration.builder().setEncoding(encoding).setSafeMode(safeMode).setDecorator(decorator).build());
	}
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final InputStream input, final Decorator decorator, final boolean safeMode) throws IOException {
		return process(input, Configuration.builder().setDecorator(decorator).setSafeMode(safeMode).build());
	}
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final InputStream input, final String encoding, final boolean safeMode) throws IOException {
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final InputStream input, final String encoding, final Decorator decorator) throws IOException {
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final InputStream input, final String encoding, final Decorator decorator, final boolean safeMode) throws IOException {
		return process(input, Configuration.builder().setEncoding(encoding).setDecorator(decorator).setSafeMode(safeMode).build());
	}
	 ...
	 * @see Configuration#DEFAULT
	 */
	public final static String process(final Reader reader, final Decorator decorator, final boolean safeMode) throws IOException {
			...
			case OTHER: {
				...
				while (line != null && !line.isEmpty) {
					...
					if (this.useExtensions && (t == LineType.CODE || t == LineType.FENCED_CODE || t == LineType.PLUGIN))
						break;
					if (t == LineType.HEADLINE || t == LineType.HEADLINE1 || t == LineType.HEADLINE2 || t == LineType.HR || t == LineType.BQUOTE || t == LineType.XML)
						break;
					line = line.next;
				}
				final BlockType bt;
